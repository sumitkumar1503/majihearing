...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'service_save') {
        ...
        $data = sheets_get($sid, 'Services!A2:C'); $found=false;
        foreach ($data as $i=>$r){ if (cell($r,0)===$oc && cell($r,1)===$od){ sheets_update($sid,'Services!A'.($i+2).':C'.($i+2),[[$cat,$desc,$price]]); $found=true; break; } }
        if (!$found) sheets_append($sid,'Services',[$cat,$desc,$price]);
...
$servicesRaw = sheets_get($sid, 'Services!A2:C');
$services = array_map(fn($r)=>['category'=>cell($r,0)?:'Service','description'=>cell($r,1),'price'=>cell($r,2)], $servicesRaw);
$config = get_config();
$CATS = ['Service','Hearing Aids','Accessories'];
$tabs = ['branches'=>'Branches','services'=>'Service Catalog','brands'=>'HA Brands','tests'=>'Medical Tests','whatsapp'=>'WhatsApp','general'=>'General'];
